...
from venusfold.model.modules.activation import apply_activation, make_activation
from venusfold.model.modules.primitives import (
    AdaptiveLayerNorm,
    Attention,
    BiasInitLinear,
...
from venusfold.model.utils import (
...
class AttentionPairBias(nn.Module):
    ...
    Args:
        ...
        condition_gate_on_s (bool, optional): add a zero-initialized s-conditioned term to attention gate logits.
            Defaults to False.
    """
    ...
    def __init__(
        ...
        condition_gate_on_s: bool = False,
    ) -> None:
        ...
        self.condition_gate_on_s = condition_gate_on_s
        if has_s:
            # Line2
        ...
        self.linear_gate_s = None
        if self.has_s and self.condition_gate_on_s:
            self.linear_gate_s = LinearNoBias(
                in_features=c_s,
                out_features=c_a,
                initializer="zeros",
    ...
    def local_multihead_attention(
        ...
        Args:
            ...
            gate_bias (torch.Tensor, optional): additive attention gate logits. Defaults to None.
    ...
    def forward(
        ...
        gate_bias = None
        if self.linear_gate_s is not None:
            gate_bias = self.linear_gate_s(s)
...
class DiffusionTransformerBlock(nn.Module):
    ...
    Args:
        ...
        condition_gate_on_s (bool, optional): add an s-conditioned term to attention gate logits.
            Defaults to False.
    """
    ...
    def __init__(
        ...
        self.attention_pair_bias = AttentionPairBias(
            ...
            condition_gate_on_s=condition_gate_on_s,
        )
...
class AtomAttentionEncoder(nn.Module):
    ...
    Args:
        ...
        local_activation (str, optional): local MLP activation. Defaults to "relu".
    """
    ...
    def __init__(
        ...
        self.small_mlp = nn.Sequential(
            make_activation(local_activation),
            LinearNoBias(
    ...
    def forward(
        ...
        if inplace_safe:
            p_lm = p_lm + self.linear_no_bias_cl(
                apply_activation(c_l_q[..., None, :], self.local_activation)
            )
            p_lm += self.linear_no_bias_cm(
                apply_activation(c_l_k[..., None, :, :], self.local_activation)
            )
            p_lm += self.small_mlp(p_lm)
        else:
            p_lm = (
                p_lm
                + self.linear_no_bias_cl(
                    apply_activation(c_l_q[..., None, :], self.local_activation)
                )
        ...
        a = aggregate_atom_to_token(
            x_atom=apply_activation(
                self.linear_no_bias_q(q_l), self.local_activation
            ),
            atom_to_token_idx=atom_to_token_idx,
            n_token=n_token,
            reduce="mean",
