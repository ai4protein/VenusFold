...
from venusfold.model.modules.transformer import AtomAttentionEncoder
...
class InputFeatureEmbedder(nn.Module):
    ...
    def forward(
        ...
        if self.esm_configs["enable"]:
            if self.training:
                logger.warning(
                    "Make sure esm_token_embedding has been cached for training. If not, refer to venusfold/data/esm/compute_esm.py first."
                )
...
class SubstructureEmbedder(nn.Module):
    ...
    def __init__(
        ...
        if self.architecture == "mlp":
            ...
            layers.append(make_activation(local_activation))
        ...
        **kwargs: Any,
        ...
        if self.substructure_embedder_config.get("enable", False):
            self.substructure_z_embedder = SubstructureEmbedder(
                ...
                local_activation=self.substructure_embedder_config.get(
                    "local_activation", local_activation
                ),
            )
