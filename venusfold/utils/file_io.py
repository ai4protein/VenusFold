...
from venusfold.data.constants import mmcif_restype_1to3
from venusfold.utils.logger import get_logger
from venusfold.utils.torch_utils import map_values_to_list, to_device
...
def read_indices_csv(
    ...
    if csv_str in _INDICES_CSV_CACHE:
        ...
        logger.info(
            "[VenusFold IO] read_indices_csv cache hit: path=%s, shape=%s",
            csv_str,
    ...
    logger.info(
        "[VenusFold IO] read_indices_csv finished in %.3fs: path=%s, shape=%s",
        time.time() - t0,
        csv_str,
        tuple(df.shape),
    )
...
def load_json_cached(path: Union[str, Path]) -> Any:
    ...
    if path_str.endswith(".json") and path_str in _JSON_FILE_CACHE:
        logger.info("[VenusFold IO] load_json_cached cache hit: path=%s", path_str)
    ...
    logger.info(
        "[VenusFold IO] load_json_cached finished in %.3fs: path=%s",
        time.time() - t0,
        path_str,
    )
    return data
